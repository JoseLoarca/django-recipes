...
class RecipesApiTests(TestCase):
    ...
    def test_partial_update_recipe(self) -> None:
        ...
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        new_tag = sample_tag(user=self.user, name='Easy')
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self) -> None:
        ...
        recipe1 = sample_recipe(user=self.user, title='Burritos')
        recipe2 = sample_recipe(user=self.user, title='Completos')
        recipe3 = sample_recipe(user=self.user, title='Carnitas')

        tag1 = sample_tag(user=self.user, name='Mexican')
        tag2 = sample_tag(user=self.user, name='Chilean')
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Hawaiian Pizza')
        recipe2 = sample_recipe(user=self.user, title='Mojito')
        recipe3 = sample_recipe(user=self.user, title='Teriyaki Chicken')

        ingredient1 = sample_ingredient(user=self.user, name='Tomatoes')
        ingredient2 = sample_ingredient(user=self.user, name='Mint Leaves')
